...
function Form({type}: Props) {
    ...
    const handleClickToHideAddedForm = (e: React.MouseEvent) => {
        $('.admin-films-background').css('left', '0%')
    }

    const handleChangeImage = (e: React.ChangeEvent) => {
        const { files }: any = e.target
        console.log(files[0])
        $('.btn-admin-add-films-image').html(files[0].name)
        setImage(URL.createObjectURL(files[0]))

    }

    return (
        <div className="admin-film-add-form">
            <i className="fas fa-angle-double-left" onClick={handleClickToHideAddedForm}></i>
            <p className="title-added-film-form">
                {type === 'add' ? 'Add Films' : 'Update Films'}
            </p>
            <form action="" className="added-film-form">
                <div className="content-added-film-form">
                    <div className="col">
                        <table>
                            <tbody>
                                <tr>
                    ...
                    <div className="col">
                        <table className="admin-added-table-right">
                            <tbody>
                                <tr className="admin-description-add-films">
                                    <th>Description:</th>
                                ...
                                <tr>
                                    <th>Image:</th>
                                    <td className="admin-added-file">
                                        <button className="btn btn-admin-add-films-image"><i className="fas fa-cloud-upload-alt"></i> Add Image</button>
                                        <input type="file" name="" id="" accept='image/*' onChange={handleChangeImage}/>
                                    </td>
                                </tr>
                                
                                <tr className="review-added-films-image">
                                    <th></th>
                                    <td >
                ...
                <div className="admin-added-films-btn-group">
